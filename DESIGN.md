...
| 意味           | 背景（10%透過）    | テキスト / チャート色                                   | 用途例                         |
| -------------- | ------------------ | ------------------------------------------------------- | ------------------------------ |
| 青（情報）     | `bg-blue-500/10`   | `text-blue-600 dark:text-blue-400` / `hsl(221 83% 53%)` | P タンパク質、睡眠チャート     |
| 黄（注意）     | `bg-yellow-500/10` | `text-yellow-600 dark:text-yellow-500`                  | F 脂質、気分（中間）、反動あり |
| 緑（良好）     | `bg-green-500/10`  | `text-green-600 dark:text-green-400` / `#16a34a`        | C 炭水化物、気分チャート       |
| 橙（高温）     | `bg-orange-500/10` | `text-orange-600 dark:text-orange-500` / `#f97316`      | カロリー、最高気温チャート     |
| 赤（警告）     | `bg-red-500/10`    | `text-red-600 dark:text-red-500`                        | 危険操作、静止不足、気分（悪） |
| シアン（低温） | `bg-cyan-500/10`   | `text-cyan-600 dark:text-cyan-400` / `#06b6d4`          | 最低気温チャート               |
...
- 反動あり：`text-yellow-500`（`FLAG_COLORS.rebound`）
- 静止できていない：`text-red-500`（`FLAG_COLORS.unstable`）
